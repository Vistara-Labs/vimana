...
usage() {
  this=$1
  cat <<EOF
$this: download go binaries for layr-labs/eigenlayer-cli
...
Usage: $this [-b] bindir [-d] [tag] [-c]
  ...
  -d turns on debug logging
   [tag] is a tag from
   https://github.com/layr-labs/eigenlayer-cli/releases
...
get_binaries() {
  case "$PLATFORM" in
    darwin/amd64) BINARIES="eigenlayer" ;;
    darwin/arm64) BINARIES="eigenlayer" ;;
    linux/amd64) BINARIES="eigenlayer" ;;
    linux/arm64) BINARIES="eigenlayer" ;;
    *)
...
completions() {
      ...
      then
        ...
        then
          ...
          then
            BASHRC=~/.bashrc
            touch $BASHRC
            sed_in_place "/.*# eigenlayer completion/d" $BASHRC
            echo "source $(brew --prefix)/etc/bash_completion # eigenlayer completion" >> $BASHRC
          else
            echo "warning: brew bash-completion package not found. eigenlayer command completion for bash not installed"
          fi
        else
          echo "warning: brew not found. eigenlayer command completion for bash not installed"
        fi
      fi
    else
      echo "warning: auto completion generation command failed. eigenlayer command completion for bash not installed"
    fi
  fi
  ...
  then
    ...
    then
      ...
      echo "fpath=($ZSH_COMPLETION_SCRIPTS_DIR \$fpath) # eigenlayer completion" >> $ZSH_COMPLETION_MAIN
      echo "rm -f ~/.zcompdump; compinit # eigenlayer completion" >> $ZSH_COMPLETION_MAIN
    else
      echo "warning: auto completion generation command failed. eigenlayer command completion for zsh not installed"
    fi
  fi
}
